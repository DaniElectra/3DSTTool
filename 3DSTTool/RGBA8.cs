            ...
            for (int By = 0; By < bitmap.Height / 8; By++)
            {
                for (int Bx = 0; Bx < bitmap.Width / 8; Bx++)
                {
                    for (int tile_count = 0; tile_count < 64; tile_count++)
                    {
                        ...
                        output[tile_count * 4 + pixel_count] = pixel_color.A;
                        output[tile_count * 4 + pixel_count + 1] = pixel_color.B;
                        output[tile_count * 4 + pixel_count + 2] = pixel_color.G;
                        output[tile_count * 4 + pixel_count + 3] = pixel_color.R;
                    }
                    pixel_count += 256;
                }
            }
            ...
            for (int By = 0; By < height / 8; By++)
            {
                for (int Bx = 0; Bx < width / 8; Bx++)
                {
                    for (int tile_count = 0; tile_count < 64; tile_count++)
                    {
                        ...
                        int alpha = input[tile_count * 4 + pixel_count];
                        byte_count++;
                        int blue = input[tile_count * 4 + pixel_count + 1];
                        byte_count++;
                        int green = input[tile_count * 4 + pixel_count + 2];
                        byte_count++;
                        int red = input[tile_count * 4 + pixel_count + 3];
                        byte_count++;
